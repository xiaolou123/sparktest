...
object Top3HotProduct {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val productClickLogsDStream = ssc.socketTextStream("hadoop01",9999)
    val categoryProductDStream = productClickLogsDStream
      .map{ productClickLog => (productClickLog.split(" ")(2)+"_"+ productClickLog.split(" ")(1), 1) }
    val categoryProductCountsDStream = categoryProductDStream.reduceByKeyAndWindow(
      (v1:Int, v2:Int) => v1+v2,
      Seconds(60),
      Seconds(10)
    )
